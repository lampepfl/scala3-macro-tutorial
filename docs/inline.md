...
As there are no direct references to `pi` and `pi2`, these definitions can be removed from the generated bytecode. There are some exceptions that we will see in [later](#inline-methods).
